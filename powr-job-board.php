<?php
    /**
     * @package POWr Job Board
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Job Board
    Plugin URI: https://www.powr.io/plugins/job-board
    Description: Post Jobs and hire fast  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Job_Board extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_job_board',
                             __( 'POWr Job Board' ),
                             array( 'description' => __( 'Job Board by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-job-board' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Job Board.
        </p>
        <p>
    ...
    function register_powr_job_board() {
      register_widget( 'Powr_Job_Board' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_job_board' );
    //Create short codes for adding plugins anywhere:
    function powr_job_board_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-job-board' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-job-board' label='$label'></div>";
      }else{
      	"<div class='powr-job-board'></div>";
      }
    }
    add_shortcode( 'powr-job-board', 'powr_job_board_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_job_board_menu' );
    function powr_job_board_menu() {
      add_menu_page( 'POWr Job Board', 'POWr Job Board', 'manage_options', 'powr-job-board-settings', 'powr_job_board_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_job_board_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/job-board';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/job-board';
      }
    ...
    register_activation_hook(__FILE__, 'powr_job_board_plugin_activate');
      add_action('admin_init', 'powr_job_board_plugin_redirect');

      function powr_job_board_plugin_activate() {
      add_option('powr_job_board_plugin_do_activation_redirect', true);
      }
      ...
      function powr_job_board_plugin_redirect() {
      if (get_option('powr_job_board_plugin_do_activation_redirect', false)) {
          delete_option('powr_job_board_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-job-board-settings&' );
          }
       }
      }
